<template>
  <div class="space-y-2 max-h-64 overflow-auto pr-1">
    <div v-for="recipe in recipes" :key="recipe.id" class="flex items-center gap-2 p-2 border border-gray-100 rounded">
    ...
    <div v-if="recipes.length === 0" class="text-sm text-gray-500 text-center py-4">선택된 요리가 없습니다</div>
  </div>
</template>
