    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div class="border border-gray-200 rounded-lg p-4 bg-white">
        <div class="flex gap-2 items-center mb-4">
          <input v-model="searchQuery" type="text" placeholder="요리 이름 검색..." class="px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent w-full" />
        ...
        <div class="space-y-3 max-h-[60vh] overflow-auto pr-1">
          <div v-for="recipe in filteredRecipes" :key="recipe.id" class="border border-gray-100 rounded p-3 hover:shadow-sm">
            <div class="flex items-start gap-3">
              <img :src="getItemImageUrl(recipe.resultItemId)" :alt="recipe.name" class="w-12 h-12 rounded object-cover" @error="handleImageError" />
              <div class="flex-1 min-w-0">
                <div class="flex items-center justify-between">
                  ...
                  <div class="flex items-center gap-2">
                    <input type="number" min="0" class="w-20 px-2 py-1 border rounded" :value="selectedCounts[recipe.id] ?? 0" @input="onChangeCount(recipe.id, ($event.target as HTMLInputElement).value)" />
                  </div>
                </div>
                <div class="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-1 text-sm">
                  <div v-for="ri in recipe.requiredItems" :key="ri.itemId" class="flex items-center gap-2">
                    <img :src="getItemImageUrl(ri.itemId)" :alt="getItemName(ri.itemId)" class="w-5 h-5 rounded object-cover" @error="handleImageError" />
      ...
      <div class="border border-gray-200 rounded-lg p-4 bg-white">
        ...
        <div class="mb-4">
          <h3 class="text-sm font-semibold text-gray-700 mb-1">주간 식재료 한도</h3>
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-64 overflow-auto pr-1">
            <div v-for="it in pricedFoodIngredients" :key="it.id" class="flex items-center gap-2">
              <img :src="it.imageUrl" :alt="it.name" class="w-5 h-5 rounded object-cover" @error="handleImageError" />
              <span class="text-sm">{{ it.name }} ({{ it.price }}G)</span>
              <input type="number" min="0" class="ml-auto w-24 px-2 py-1 border rounded" :value="weeklyLimits[it.id] ?? 0" @input="onChangeWeeklyLimit(it.id, ($event.target as HTMLInputElement).value)" />
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div class="space-y-1 max-h-64 overflow-auto pr-1">
            <div v-for="row in totalNeededRows" :key="row.itemId" class="flex items-center gap-2 text-sm">
              <img :src="getItemImageUrl(row.itemId)" :alt="getItemName(row.itemId)" class="w-5 h-5 rounded object-cover" @error="handleImageError" />
              <span class="truncate">{{ getItemName(row.itemId) }}</span>
              <span class="ml-auto">x{{ row.quantity }}</span>
              <span v-if="row.price" class="w-24 text-right text-gray-500">{{ row.price }}G</span>
            </div>
          </div>
        </div>

        <div class="border-t pt-3 mt-3 flex items-center justify-between">
          <div class="text-sm text-gray-700">총 비용</div>
          <div class="text-lg font-bold">{{ totalPrice }} G</div>
        </div>
      </div>
    </div>
  </div>
...
const facilityLevels = computed(() => {
  ...
  recipes.forEach(r => set.add(r.facilityLevel))
...
const pricedFoodIngredients = computed<Item[]>(() => {
  return items.filter(i => i.category === ITEM_CATEGORY.FOOD_INGREDIENT && typeof i.price === 'number')
})
...
function onChangeWeeklyLimit(id: string, val: string) {
  const n = Math.max(0, Number(val) || 0)
  weeklyLimits.value[id] = n
}
...
const totalPrice = computed(() => {
  let sum = 0
  totalNeededRows.value.forEach(row => {
...
const maxCraftableCount = computed(() => {
  // 필요한 식재료 중 가격이 있고 식재료 카테고리인 것만 한도로 제한
