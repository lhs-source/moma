<template>
  <div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <h1 class="text-3xl font-bold">아이템 목록</h1>
      ...
      >
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1 min-w-0">
            ...
            <div v-if="item.craftingRecipe && item.craftingRecipe.length > 0" class="mt-2">
              <p class="text-xs font-medium text-gray-700">제작 재료:</p>
              <div class="text-xs text-gray-600 mt-1">
                <span
                  v-for="(material, index) in item.craftingRecipe"
                  :key="material.itemId"
                >
                  {{ getItemName(material.itemId) }} x{{ material.quantity }}{{ index < item.craftingRecipe.length - 1 ? ', ' : '' }}
                </span>
              </div>
            </div>
          </div>
