...
기존 데이터 목록(아이템/레시피/NPC/Location 탭), 제작, 요리 시뮬레이팅, 물물교환, 이벤트 캘린더 시스템의 사용성을 개선하고, 누락된 편의 기능을 추가하여 사용자 경험을 향상시킵니다. 현재 구현된 기능들을 안정화하고, 모바일 반응형과 성능을 개선하며, 일관된 UI/UX를 제공합니다. NPC와 Location 데이터를 추가하여 더욱 포괄적인 게임 정보를 제공하고, 멀티 캐릭터 환경에서의 재료 관리, 가공 시뮬레이션, 주간 식재료 구매 한도를 고려한 요리 시뮬레이션 기능을 추가하여 효율적인 게임 플레이를 지원합니다. 이 Epic은 기존 시스템의 기초를 탄탄히 하여 이후 Epic들의 토대를 마련합니다.
...
So that 특정 지역의 채집 가능 아이템이나 특수 이벤트를 빠르게 찾을 수 있다.

**Acceptance Criteria:**
1. 데이터 목록에 Location 탭이 추가되어야 함
2. 모든 Location 정보가 목록으로 표시되어야 함 (지역명, 설명)
3. Location 이름으로 검색할 수 있어야 함
4. Location별 채집 가능 아이템이 연관 표시되어야 함
5. Location 상세 정보를 클릭으로 확인할 수 있어야 함
6. 모바일에서도 반응형으로 표시되어야 함
7. Location 데이터가 로컬에서 관리되어야 함
8. Location 이미지나 지도가 표시되어야 함 (있는 경우)
