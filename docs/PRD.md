...
- **FR017:** 시스템은 요리, 제작, 포션 제조, 수공예 등 모든 제작 레시피를 제공해야 한다
- **FR018:** 사용자는 제작하고 싶은 아이템을 선택하면 필요한 모든 재료를 계산해야 한다
- **FR019:** 시스템은 재료의 재귀적 의존성을 계산해야 한다 (재료를 만들기 위한 재료까지)
- **FR020:** 사용자는 멤버십 여부를 선택하여 제작 가능 여부를 확인할 수 있어야 한다
- **FR021:** 시스템은 가공 레시피 정보를 제공해야 한다 (직물, 식품, 가죽, 약품, 금속, 목재)
- **FR022:** 시스템은 멀티 캐릭터 환경에서 각 캐릭터별 보유 재료를 독립적으로 관리하고 가공 가능 수량을 계산해야 한다

#### 요리 시뮬레이션 시스템

- **FR023:** 시스템은 모든 요리 레시피와 필요한 식재료 정보를 제공해야 한다
- **FR024:** 사용자는 요리하고 싶은 아이템을 선택하면 필요한 식재료를 계산해야 한다
- **FR025:** 시스템은 각 식재료의 주간 구매 가능 수량 정보를 표시해야 한다
- **FR026:** 사용자가 여러 요리를 계획하면 총 필요 식재료를 집계하고 주간 구매 한도 내 실현 가능 여부를 판단해야 한다
- **FR027:** 시스템은 주간 구매 한도를 초과하는 식재료를 강조 표시하고 대체 방안을 제시해야 한다
- **FR028:** 시스템은 캐릭터별 주간 식재료 구매 계획을 저장하고 불러올 수 있어야 한다

#### 물물교환 시스템

- **FR029:** 시스템은 NPC별 물물교환 가능한 아이템 목록을 제공해야 한다
- **FR030:** 사용자는 원하는 물물교환 아이템을 선택하면 필요한 재료를 계산해야 한다
- **FR031:** 시스템은 주간 물물교환 가능한 아이템을 별도로 표시해야 한다
- **FR032:** 사용자는 여러 물물교환을 계획하면 총 필요 재료를 집계해야 한다
- **FR033:** 시스템은 사용자의 물물교환 계획을 로컬 스토리지에 저장하고 불러올 수 있어야 한다

#### 이벤트 및 일정 관리

- **FR034:** 시스템은 진행 중인 모든 이벤트 정보를 제공해야 한다
- **FR035:** 시스템은 이벤트 종료일까지 남은 기간을 시각적으로 표시해야 한다
- **FR036:** 시스템은 캐시샵 한정 아이템 판매 일정을 제공해야 한다
- **FR037:** 시스템은 월별 캘린더 뷰와 주간 캘린더 뷰를 제공하고, 사용자가 뷰를 전환할 수 있어야 한다
- **FR038:** 주간 캘린더 뷰는 선택된 주의 7일을 표시하고, 이전/다음 주로 이동할 수 있어야 한다
- **FR039:** 월별 및 주간 캘린더 모두 동일한 이벤트 바 UI로 일관성 있게 표시해야 한다
- **FR040:** 주간 뷰에서도 이벤트 바는 시작일/종료일을 시각적으로 표현하고 클릭하여 상세 정보를 확인할 수 있어야 한다
- **FR041:** 시스템은 정기 업데이트 주기를 반영하여 일정을 관리해야 한다 (수요일 공지, 목요일 업데이트 적용)

#### 데이터 관리 및 정확성

- **FR042:** 시스템은 모든 데이터를 로컬에서 관리해야 한다 (공식 API 없음)
- **FR043:** 관리자는 아이템, 레시피, 이벤트 데이터를 추가/수정할 수 있어야 한다
- **FR044:** 시스템은 아이템 이미지를 표시할 수 있어야 한다

#### UI/UX

- **FR045:** 시스템은 반응형 디자인을 제공하여 모바일에서도 사용 가능해야 한다 (PC 우선)
...
- **핵심 화면**: 홈, 데이터 목록 (아이템/레시피/NPC/Location 탭), 제작, 요리 시뮬레이팅, 물물교환, 이벤트 캘린더 (월별/주간 뷰)
- **네비게이션**: 상단 메뉴 또는 사이드바를 통한 주요 섹션 이동
- **검색**: 데이터 목록 내 검색 기능 및 전역 검색을 통한 빠른 아이템 접근
- **탭 구조**: 데이터 목록 메뉴에서 아이템, 레시피, NPC, Location 등을 탭으로 구분하여 제공
...
**목표**: 기존 데이터 목록(아이템/레시피/NPC/Location), 제작, 요리 시뮬레이팅, 물물교환, 이벤트 캘린더 시스템의 사용성을 개선하고 누락된 편의 기능을 추가  
**예상 스토리 수**: 16개  
**진행 상황**: 2/16 완료 (Story 1.2 ✅, Story 1.14 ✅)

주요 작업: 검색 성능 최적화, 스키마별 아이템 목록 표시, 필터링 옵션 확장, 탭 기반 데이터 네비게이션 (NPC/Location 탭 추가), 모바일 반응형 개선, 사용자 인터페이스 일관성 개선, 데이터 로딩 상태 표시, 에러 핸들링 개선, 멀티 캐릭터 가공 시뮬레이션, 주간 식재료 기반 요리 시뮬레이션, ~~이벤트 캘린더 주간 뷰 추가~~ ✅, ~~스키마별 아이템 목록 표시 및 선택~~ ✅
