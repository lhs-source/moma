...
- **FR031:** 시스템은 월별 캘린더 뷰와 주간 캘린더 뷰를 제공하고, 사용자가 뷰를 전환할 수 있어야 한다
- **FR032:** 주간 캘린더 뷰는 선택된 주의 7일을 표시하고, 이전/다음 주로 이동할 수 있어야 한다
- **FR033:** 월별 및 주간 캘린더 모두 동일한 이벤트 바 UI로 일관성 있게 표시해야 한다
- **FR034:** 주간 뷰에서도 이벤트 바는 시작일/종료일을 시각적으로 표현하고 클릭하여 상세 정보를 확인할 수 있어야 한다
- **FR035:** 시스템은 정기 업데이트 주기를 반영하여 일정을 관리해야 한다 (수요일 공지, 목요일 업데이트 적용)

#### 데이터 관리 및 정확성

- **FR036:** 시스템은 모든 데이터를 로컬에서 관리해야 한다 (공식 API 없음)
- **FR037:** 관리자는 아이템, 레시피, 이벤트 데이터를 추가/수정할 수 있어야 한다
- **FR038:** 시스템은 아이템 이미지를 표시할 수 있어야 한다

#### UI/UX

- **FR039:** 시스템은 반응형 디자인을 제공하여 모바일에서도 사용 가능해야 한다 (PC 우선)
...
- **핵심 화면**: 홈, 데이터 목록 (아이템/레시피 탭), 제작, 요리 시뮬레이팅, 물물교환, 이벤트 캘린더 (월별/주간 뷰)
...
**예상 스토리 수**: 14개  
**진행 상황**: 1/14 완료 (Story 1.14 ✅)

주요 작업: 검색 성능 최적화, 스키마별 아이템 목록 표시, 필터링 옵션 확장, 탭 기반 데이터 네비게이션, 모바일 반응형 개선, 사용자 인터페이스 일관성 개선, 데이터 로딩 상태 표시, 에러 핸들링 개선, 멀티 캐릭터 가공 시뮬레이션, 주간 식재료 기반 요리 시뮬레이션, ~~이벤트 캘린더 주간 뷰 추가~~ ✅
